...
<body class="bg-body">
	...
	<div class="index-wall white " style=" ">
		<div class="container" style=" position:relative;padding-left: 100px;">
			<form class="form-inline"
				...
				<div class="form-group mar-left-10">
					<label for="">出发日期</label> <input style="cursor: pointer;width:110px;" type="text" class="form-control "
						name="starttime"  id=""
						value="<fmt:formatDate value="<%=new Date()%>"/>" placeholder="出发"
						onClick="WdatePicker()">
				</div>
				<div class="form-group mar-left-10">
					<label for="dropAirlines">航空公司</label> <select id="dropAirlines"
						class="form-control" style=" width:120px;" name="aircompany">
					</select>
				</div>
	...
	<div class="container mar-bottom-30 ">
		...
		${msg }
			<div>
				<c:forEach var="all" items="${ findAll.data}">
					<!-- 表头 -->
					<ul class="list-inline bor-bottom-solid-1  ">
						<li class="w-percentage-25"><img
							src="${pageContext.request.contextPath}/img/air/3U.png"
							width="24" height="24"> <strong>
								${all.flight.aircompany }</strong> <span
							class="gray-999 font12 mar-left-10">机型：${all.flight.airtype }</span></li>
						<li class="text-right w80"><strong class="time ">${all.flight.starttime  }</strong></li>
						<li class="">——</li>
						<li class="w80"><strong class="time ">${all.flight.endtime }</strong></li>
						<li class="w100 text-right">${all.flight.startairport }</li>
						<li class="">——</li>
						<li class=" w100">${all.flight.endairport }</li>
					</ul>
					<!-- 表头结束 -->
					<!-- 表BODY -->
					<c:set var="airtickets" value="${all.airtickets }"></c:set>
					<c:forEach var="ticket" items="${airtickets}">
						<div class="collapse " id="collapseExample2"
							style=" display:block;">
							<div class="hangbanlist-body " style=" background-color:#FEFCFC;">
								<ul class="list-inline">
									<li class="w-percentage-20"><strong class="blue-0093dd">${ticket.seattype }</strong></li>
									<li class="w-percentage-25">座位数：${ticket.seatnum}</li>
									<li class="w-percentage-25">票面价：<span class="rmb">￥${ticket.price }</span></li>
									<li class="pull-right "><button type="button" 
											class="btn btn-danger btn-sm"
											onClick="window.location.href ='${pageContext.request.contextPath}/need/buying?method=buyTicket&ticketid=${ticket.id }';">订票</button></li>
								</ul>
							</div>
							<!-- 表BODY 结束 -->
						</div>
				...
				<ul class="pagination">
					<li><a href="${requestScope['javax.servlet.forward.request_uri']}?method=${param.method }&currentPage=1">首页</a></li>
     <c:forEach  var="pagenum" begin="${findAll.start }" end="${findAll.end }" >
    <!-- 获取 地址栏  param.name获取地址栏参数值-->
    <c:if test="${pagenum!=0 }">
    <li><a href="${requestScope['javax.servlet.forward.request_uri']}?method=${param.method }&currentPage=${pagenum}">${pagenum }</a></li>
    </c:if>
    </c:forEach>
    <li><a href="${requestScope['javax.servlet.forward.request_uri']}?method=${param.method }&currentPage=${findAll.end }">尾页</a></li>
				</ul>
				</nav>
				<!-- 分页结束 -->
